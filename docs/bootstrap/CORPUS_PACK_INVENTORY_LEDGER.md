...
| Removed non-cleared PDF sample | removed local sample artifact | Local sample artifact | about 720 KB observed before removal | unknown | Historical PDF sample | evaluation | no | no | no | no | no | no | none | legal | exclude | Removed from the public release branch unless source and redistribution status are clear. |
| Private third-party reference anchors | private/operator-provided only; public boundary at `reports/reference_anchors/README.md` | Abstracted evaluation anchors | about 5 KB observed before removal | excluded | Non-media local audit anchor aid | scaffold_evidence | no | no | no | no | no | no | none | legal | exclude | Copyrighted third-party anchors are not shipped; future public fixtures must be fictional, owned, synthetic, or permissively licensed. |
