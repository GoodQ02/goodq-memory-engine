...
3. **Check results:**
   ```powershell
   # View logs
   Get-Content <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\logs\step_runs.jsonl -Tail 20

   # Check database
   sqlite3 <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\data\memory_db\memory.db "SELECT COUNT(*) FROM scenes;"
   ```
...
**Find errors:**
```powershell
Get-Content <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\logs\step_runs.jsonl | `
...
# Edit <project_root>\configs\config_open.yaml
# tools:
#   ffmpeg_exe: "<project_root>\\tools\\ffmpeg\\bin\\ffmpeg.exe"
```
...
Get-Content <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\logs\step_runs.jsonl -Tail 100 | `
...
```powershell
Get-ChildItem "<GOODQ_DATA_ROOT>\videos\archive" -Filter *.mp4 | ForEach-Object {
    Write-Host "Processing $($_.Name)..." -ForegroundColor Cyan
    
    Copy-Item $_.FullName <project_root>\import_inbox\
    ...
    Move-Item $_.FullName "<GOODQ_DATA_ROOT>\videos\processed\"
    
    # Clean inbox
    Remove-Item <project_root>\import_inbox\* -Force
}
```
...
Copy-Item <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\data\memory_db\*.index $backupDir\
Copy-Item <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\data\memory_db\*.sqlite $backupDir\
...
# View summary
Get-Content <GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\logs\health_check.log -Tail 50
```
...
| Logs | `<GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\logs\` |
| Database | `<GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\data\memory_db\memory.db` |
| FAISS | `<GOODQ_DATA_ROOT>\GoodQ_Data (See LEGACY_PATHS_DEPRECATED.md)\data\memory_db\*.index` |
| Models | `<GOODQ_DATA_ROOT>\models (See LEGACY_PATHS_DEPRECATED.md)\` |
...
|----------|---------|---------|
| `HF_HOME` | HuggingFace cache | `<GOODQ_DATA_ROOT>/models` |
| `TORCH_HOME` | PyTorch cache | `<GOODQ_DATA_ROOT>/models` |
