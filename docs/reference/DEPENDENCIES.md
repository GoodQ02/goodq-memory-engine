...
| Legacy memory-tier alias (`chroma` -> `ephemeral`) | Compatibility residue | The runtime keeps a compatibility alias for old `chroma` tier names, but the active tier-0 cache is an in-memory ephemeral store rather than ChromaDB. | `steps/common/memory_store.py:6`; `steps/common/memory_store.py:49`; `steps/common/memory_stores.py:18`; `steps/common/memory_stores.py:394` |
...
| Face identity capability pack (OpenCV YuNet/SFace) | Model/package | Verified bundled-only ONNX payload; dlib is a visible degraded fallback. | `docs/reference/FACE_EMBEDDING_POLICY.md`; `steps/face_embed/step.py` |
| WSL audio acceleration stack (`faster-whisper`, `pyannote.audio`, CUDA torch) | Model/package | Canonical accelerated audio path runs through the WSL unified worker and may degrade gracefully when diarization is unavailable. Bootstrap package authority is `wsl2_audio/requirements-bootstrap-constraints.txt`, including the `pyannote.audio==3.3.2` / `huggingface-hub==0.35.3` compatibility pair. | `wsl2_audio/process_audio.py:652`; `wsl2_audio/process_audio.py:700`; `scripts/wsl_audio_preflight.py:166` |
