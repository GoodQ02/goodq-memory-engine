...
|---|---|---|---|---|
| `GPU_ENHANCED` runtime profile | A GPU-labelled installer rejects a CPU-only Torch runtime and requires a CUDA-compiled runtime with a visible device at admission; `BASELINE` remains CPU-safe. | Required in GPU Enhanced; optional in CPU baseline | CUDA-validated when explicitly selected | `AGENTS.md:19`, `AGENTS.md:45`, `configs/config.yaml:139`, `configs/config.yaml:140`, `scripts/install/verify_offline_suite.ps1` |
