...
def _validate_public_template(evidence: dict[str, Any]) -> None:
    """Allow an anonymous empty template, never a weakened runtime receipt."""
    authority = evidence["authority"]
    if (authority.get("profile") != "PUBLIC_RELEASE_UNCONFIGURED"
            or authority.get("config_source") != "public_unconfigured_template"
            or not authority["epoch_id"].startswith("epoch_public_")
            or evidence.get("historical_evidence")):
        raise ValueError("public template must have anonymous unconfigured authority")

    def empty_counts(value: Any) -> bool:
        if isinstance(value, dict):
            return all(empty_counts(item) for item in value.values())
        if isinstance(value, list):
            return not value
        return type(value) is int and value == 0

    persistence = dict(evidence["persistence"])
    qdrant = persistence.pop("qdrant", {})
    if not empty_counts(evidence["completion"]) or not empty_counts(persistence):
        raise ValueError("public template cannot carry observed corpus counts or artifacts")
    if qdrant.get("state") != "not_probed":
        raise ValueError("public template cannot claim Qdrant readiness")
    collections = qdrant.get("collections", {})
    if set(collections) != set(MODALITIES):
        raise ValueError("public template requires all four placeholder collections")
    for modality, item in collections.items():
        expected = {"name": _expected_collections(authority["epoch_id"])[modality],
                    "status": "not_probed", "points_count": 0, "dimensions": 0}
        if item != expected:
            raise ValueError("public template cannot carry observed collection evidence")
    extra_qdrant = {k: v for k, v in qdrant.items() if k not in {"state", "collections"}}
    if not empty_counts(extra_qdrant) or any(
        item != {"state": "not_probed"} for item in evidence["observed_services"].values()
    ):
        raise ValueError("public template cannot carry service observations")
    for name in ("ollama", "vllm"):
        model = evidence["configured_runtime"].get(name, {})
        if model.get("model") or model.get("endpoint"):
            raise ValueError("public template cannot carry configured model routes")


def _render_public_template(evidence: dict[str, Any]) -> str:
...
- Template identity: `{evidence['evidence_id']}`
- Template date: `{evidence['captured_at_utc']}`
- Placeholder epoch: `{evidence['authority']['epoch_id']}`
...
def validate_evidence(evidence: dict[str, Any]) -> None:
    ...
    if kind not in (None, "unconfigured_template"):
        raise ValueError("unsupported current-state evidence kind")
    template = kind == "unconfigured_template"
    if template:
        _validate_public_template(evidence)
    qdrant = evidence["persistence"].get("qdrant")
    if not isinstance(qdrant, dict) or qdrant.get("state") != ("not_probed" if template else "running_loopback"):
    ...
    for modality in MODALITIES:
        ...
        if item.get("status") != ("not_probed" if template else "green"):
    ...
    *,
    ...
    return {
        ...
        **({"evidence_kind": "unconfigured_template"}
           if evidence.get("evidence_kind") == "unconfigured_template" else {}),
...
def render_current_state_markdown(evidence: dict[str, Any]) -> str:
    validate_evidence(evidence)
    if evidence.get("evidence_kind") == "unconfigured_template":
        return _render_public_template(evidence)
