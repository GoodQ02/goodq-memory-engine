...
& conda run -n $envName python -m goodq4all.cli.memory rebuild-id-maps | Write-Host

Write-Host "[reconcile] Cleaning placeholder rows (if any)" -ForegroundColor Cyan
& conda run -n $envName python -m goodq4all.cli.memory cleanup-placeholders | Write-Host

Write-Host "[reconcile] Running memory health check via CLI" -ForegroundColor Cyan
$out = & conda run -n $envName python -m goodq4all.cli.memory health-check
$code = $LASTEXITCODE
Write-Host $out
if ($code -ne 0) {
