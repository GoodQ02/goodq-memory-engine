...
if errorlevel 1 (
    echo ERROR: Failed to run Python in conda environment 'goodq_zenml'
    echo.
...
echo ? Environment reachable: goodq_zenml
echo.

echo [Step 2/3] Running Python path configuration test...
"%CONDA_EXE%" run -n goodq_zenml python test_python_paths.py
