...
def test_recovery_database():
    ...
    print("=" * 70)
    print("[SYMBOL] Testing Recovery Database")
    print("=" * 70)
    ...
    print(f"   [OK] Failure recorded: ID #{failure_id}")
    ...
    print(f"   [OK] Attempt recorded: ID #{attempt_id}")
    ...
    print(f"   [OK] Success recorded: ID #{attempt_id}")
    ...
    print(f"   [OK] Found {len(similar)} similar failures")
    ...
    if best:
        print(f"   [OK] Best strategy: {best.get('strategy', 'N/A')}")
    ...
    print(f"   [OK] Total failures: {stats['total_failures']}")
    print(f"   [OK] Resolved: {stats['resolved_failures']}")
    print(f"   [OK] Resolution rate: {stats['resolution_rate']*100:.1f}%")
    
    db.close()
    print("\n[OK] Recovery database tests complete!")


def test_control_agent_recovery():
    """Test Control Agent with recovery features"""
    print("\n" + "=" * 70)
    print("[BOT] Testing Control Agent Recovery Features")
    print("=" * 70)
    ...
    print(f"   {'[OK]' if success else '[FAIL]'} Recovery {'succeeded' if success else 'failed'}")
    ...
    if suggestions:
        print(f"\n   [TIP] Preventive Suggestions:")
        for i, sug in enumerate(suggestions[:3], 1):
            print(f"      {i}. {sug['suggestion'][:100]}...")
    else:
        print("   ℹ️  No suggestions yet (need more data)")
    
    print("\n[OK] Control Agent recovery tests complete!")


def main():
    """Run all tests"""
    print("\n" + "=" * 70)
    print("[SYMBOL] GoodQ4All Phase 2: Recovery System Tests")
    print("=" * 70)
    ...
    try:
        ...
        print("=" * 70)
        print("\n[LOG] Summary:")
        print("   [OK] Recovery database functional")
        print("   [OK] Failure recording working")
        print("   [OK] Recovery attempt tracking working")
        print("   [OK] AI diagnosis integration working")
        print("   [OK] Statistics and learning working")
        print("\n[LAUNCH] Phase 2 is ready for production integration!")
