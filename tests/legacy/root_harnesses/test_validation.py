...
try:
    cfg = load_configs({})
    print(f"  [SYMBOL] Config loaded (type: {type(cfg).__name__})")
    ...
    print(f"[SYMBOL] Config load failed: {e}\n")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
if os.path.exists(test_video):
    ...
    print(f"  [SYMBOL] Test video found: sample.mp4 ({size_mb:.2f} MB)\n")
else:
    print(f"  [SYMBOL] Test video not found: {test_video}\n")
    sys.exit(1)
...
print(f"  [SYMBOL] Processing root exists\n")
...
try:
    ...
    print("  [SYMBOL] Ingestion completed")
    print(f"  Result type: {type(result)}")
    if isinstance(result, dict):
        print(f"  Video ID: {result.get('video_id', 'N/A')}")
    print()
except Exception as e:
    print(f"  [SYMBOL] Ingestion failed: {e}")
    import traceback
    traceback.print_exc()
    print()
...
if os.path.exists(processing_dir):
    print(f"  [SYMBOL] Processing directory exists: {video_id}")
    ...
    if os.path.exists(temporal_index):
        ...
        with open(temporal_index) as f:
            ti = json.load(f)
        print(f"  [SYMBOL] Temporal index exists")
        print(f"    Phase 5: {ti.get('phase5_complete', False)}")
        print(f"    Phase 6: {ti.get('phase6_complete', False)}")
        print(f"    Scenes: {len(ti.get('scenes', []))}")
    else:
        print(f"  [SYMBOL] Temporal index missing")
    
    if os.path.exists(scene_manifest):
        print(f"  [SYMBOL] Scene manifest exists")
    else:
        print(f"  [SYMBOL] Scene manifest missing")
else:
    print(f"  [SYMBOL] Processing directory not found: {processing_dir}")
