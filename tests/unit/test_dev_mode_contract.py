...
def test_vllm_launcher_waits_for_the_advertised_speed_endpoint_not_only_systemd():
    wrapper = (REPO_ROOT / "scripts" / "start_vllm_servers.bat").read_text(encoding="utf-8").lower()
    assert 'start_vllm_servers.ps1" %*' in wrapper
    assert "exit /b %vllm_start_exit_code%" in wrapper
    # The executed readiness and failure paths live in test_vllm_start_control.
